...
# all primary schools (PS, PSIS) within project district and subdistrict
  def self.ps_schools_in_subdistricts(subdistrict_pairs)
    where("(district, subdistrict) IN (VALUES #{subdistrict_pairs.join(',')})").where("org_level ILIKE '%PS%'")
  end

# all intermediate schools (IS, PSIS, ISHS) within project district and subdistrict
  def self.is_schools_in_subdistricts(subdistrict_pairs)
    where("(district, subdistrict) IN (VALUES #{subdistrict_pairs.join(',')})").where("org_level ILIKE '%IS%'")
  end
