...
# array of objects --> district & subdistrict info
  def set_subdistricts
    subdistricts = Db::SchoolSubdistrict.intersecting_with_bbls(project.bbls_geom)
...
# array of objects --> bluebook is one type of schools database
  def set_bluebook
    ...
    ps_schools = Db::BluebookSchool.ps_schools_in_subdistricts(subdistrict_pairs)
    is_schools = Db::BluebookSchool.is_schools_in_subdistricts(subdistrict_pairs)
    hs_schools = Db::BluebookSchool.high_schools_in_boro(subdistrict_pairs)
...
# array of objects --> lcgms is another type of schools database
  def set_lcgms
    ...
    subdistricts.each do |subdistrict|
      ...
      lcgms_schools = Db::LcgmsSchool.lcgms_intersecting_subdistrict_geom(subdistrict_geom)
...
# array of objects --> schools that are currently under construction by the SCA
  def set_sca_projects
    ...
    subdistricts.each do |subdistrict|
      ...
      sca_schools = Db::ScaCapitalProject.sca_projects_intersecting_subdistrict_geom(subdistrict_geom)
...
# array of objects with "multiplier" values based on subdistrict, district, & level
  def set_future_enrollment_multipliers
    # subdistrict_pairs are e.g. "(<district>, <subdistrict>)" and defined in private methods
    enrollment_pct_by_sd = Db::EnrollmentPctBySd.enrollment_percent_by_subdistrict(subdistrict_pairs)
...
# array of objects --> number of high school students projected in a borough by a certain year
  ...
  enrollment_projection_by_boro = Db::EnrollmentProjectionByBoro.enrollment_projection_by_boro_for_year(buildYearMaxed.to_i, project.borough)
  #
...
def set_future_enrollment_projections
  ...
  enrollment_projection_by_district = Db::EnrollmentProjectionBySd.enrollment_projection_by_subdistrict_for_year(buildYearMaxed, districts)
...
def set_hs_students_from_housing
  high_school_students_from_housing = Db::HousingPipelineByBoro.high_school_students_from_new_housing_by_boro(project.borough)

  hs_students = high_school_students_from_housing.map &:hs_students
...
def set_future_enrollment_new_housing
  # subdistrict_pairs are e.g. "(<district>, <subdistrict>)" and defined in private methods
  future_enrollment_new_housing_by_subdistrict = Db::HousingPipelineBySd.ps_is_students_from_new_housing_by_subdistrict(subdistrict_pairs)
...
def set_doe_util_changes
  ...
  doe_significant_utilization_changes = Db::DoeSignificantUtilizationChange.doe_util_changes_matching_with_building_ids(buildingsBldgIds)
